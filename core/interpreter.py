...
from iris.core.constants import (IF_STMNT, ELIF_STMNT, ELSE_STMNT, CONDITION,
                                 ...
                                 EXPRESSION, FUNCTION, RETURN_TOKEN)
...
def compile_from_source(raw, curr_eid=None, curr_context=None):
    ...
    raw_blocks = cut_blocks(raw)
    cooked_blocks = []

    for block in raw_blocks:
        compiled_block = compile_block(curr_eid, block, curr_context, execute_blocks)
...
def execute_block(block, context):
    if block.type == VAR_ASSIGN:
        ...
        return execute_blocks(condition.body, context)
...
def execute_blocks(blocks, context=mainspace_context):
    for block in blocks:
        ...
        response = execute_block(block, context)
...
def interpret(source):
    blocks, _ = compile_from_source(compose(source))
